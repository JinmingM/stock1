...
var stock={
	...
	addNum:function(num1, num2) {
		...
		//////console.log.log.log("addNum");
		return (num1 * m + num2 * m) / m;
	},
	header:function(){
		////////console.log.log.log.log("hearder");
		var data=this.data;
		$("#name").html(data[0]);
		//(当前价-昨收价)/昨收价
		var range=(data[3]-data[2])/data[2];
		////////console.log.log.log.log(data);
		if(data[3]==0){
			range=0;
			data[3]=data[2];
		}
	...
	hq:{
		...
		//是否交易时间内
		isRun:function(){
			////console.log.log.log("isRun");
			////console.log.log.log(stock.day);
			////console.log.log.log(stock.data[30]);
		...
		reload:function(c,o){
			...
			$.getScript("http://hq.sinajs.cn/list="+stock.stockType+stock.stockID).done(function(){ 
				...
				//////console.log.log.log(stock.data);
				//更新顶部
				stock.header();
		...
		run:function(){
			////console.log.log.log("hq:run");
			var index=0;
			
			//启动线程
			window.setInterval(function(){
				////console.log.log.log(stock.hq.isRun());
				if(stock.hq.isRun()){
					index++;
					stock.hq.reload(function(result){
						////console.log.log.log("897243578923478583427852345");
						//////console.log.log.log("result:"result);
						if(1){
							////console.log.log.log($("#minute"));
							if($("#minute").length==1){
								////console.log.log.log("jkdsjcjsadcjanscnkascsadcsadc");
	...
	error:function(msg,top){
		...
		if($("#error").length==0){
			$("#stockTrendMap").append('<div id="error"><div></div></div>');
		}
		$("#error").show();
	...
	load:function(data){
		...
		if(this.isPC){
			$("#stockTrendMap").css({width:800,height:500})
		}else{
			$(window).bind('resize load', function() {
				$("#stockTrendMap").css("zoom", $(window).width() / 640);
			});
		}
	...
	menu:{
		...
		//点击
		click:function(){
			////console.log.log.log("menu:click");
	...
	tpl:{
		...
		//顶部
		header:function(){
			////console.log.log.log("tpl:header");
			var str="";
			...
			$("#stockTrendMap").append(str);
			//释放内存
			delete str;
		},
		//菜单
		menu:function(data){
			////console.log.log.log("tpl:menu");
			var str="";
			str+='<div id="nav"><ul>';
			for(var i=0;i<1;i++){
		...
		minute:function(name){
			////console.log.log.log("tpl:minute");
			if($("#"+name).length==0){
				var str="";
	...
	script:{
		jspath:function(){
			////console.log.log.log("script:jspath");
			var js="stock.js";
		...
		isPC:function() {
			////console.log.log.log("script:isPC");
		...
		reload:function(name){
			////console.log.log.log("script:reload");
			this.load(name);
		},
		//JS入口
		load:function(name){
	        ////console.log.log.log("script:load");
			this.include.load(name,function(){
				...
				eval("stock_"+name).load(name,function(result){
					...
					//画图
					////console.log.log.log("name1:"+name);
					// /..canvas();
					//eval("stock_"+name).canvas(function(){
						...
						//$("#loading").hide();//隐藏加载框
					//});
					////console.log.log.log("name2:"+name);
					delete name;
				});
			});	
		...
		include:{
			...
			load:function(name,c,o){
				////console.log.log.log("include:load");
				if(eval("this.jslist._"+name)==undefined){
					eval("this.jslist._"+name+"='"+name+"'");
					if(name=="minute"){
						////console.log.log.log(stock.jspath);
					
						
					}else{
						//////console.log.log.log('http://jieone.com/demo/stock/data/stock.php?Action=loadjs&name='+name);
...
var stock_minute1={
		...
		load:function(name,c,o){
			...
			stock.ajax({},function(result,data){
				////console.log.log.log("syock_minute1:load");
				if(!result||data==""){
					c.call(o,false);
				}else{
					data=data.split("\n");
		...
		//画图
		canvas:function(){
			////console.log.log.log("canvas");
			this.dispose();//先处理数据
			this.k.load();//加载K线图
			this.l.load();//加载K线图
			this.sellbuy();
			////console.log.log.log("ay");
			//只加载一次
			if(!this.istouch){
				this.istouch=true;
				this.touch.load();
		...
		update:function(){
			...
			////console.log.log.log("stock_minute1.update");
	...
	sellbuy:function(){
		var data=stock.data;
		//卖
		////console.log.log.log(stock_minute1.p_price);
